...
int twi_read_byte(uint8_t sla, uint8_t sub, uint8_t *buf);
int twi_write_byte(uint16_t sla, uint8_t sub, uint8_t data);
 ...
 * System clock in Hz.
 */
// #define F_CPU 12000000UL	/* Note [2] */
...
#define TWI_SLA_L3G4200D 0b11010000
#define TWI_SLA_LIS3MDL  0b00111000
...
ioinit(void)
{
   ...
   */
  UCSRA = _BV(U2X);
  UBRR = (F_CPU / (8 * 38400UL)) - 1; /* 9600 Bd */
#else
  UCSRA = 0;
  UBRR = (F_CPU / (16 * 38400UL)) - 1; /* 9600 Bd */
#endif
...
#else
  TWBR = (F_CPU / 200000UL - 16) / 2;
#endif

//  DDRC &= ~(_BV(0) | _BV(1)); //as output
...
int twi_read_bytes(uint8_t sla, uint8_t sub, uint8_t len, char* buf) {
  uint8_t twcr, n = 0;
  int rv = 0;
...
int twi_write_bytes(uint16_t sla, uint8_t sub, int len, uint8_t *buf)
{
  uint8_t n = 0;
  int rv = 0;
...
int main(void)
{
  ...
  stdout = &mystdout;
  printf("%s", "avrinit()\n");  
  for(a=0;a<10;a++) uart_putchar('\n',0);

  compass_init(TWI_SLA_LIS3MDL);
  printf("%s", "compass()\n");  
...
//    yaw = 0;
    ...
    for (a=0;a<cpf;a++) {
      ...
      while (!(status & GYRO_ZDA_REG)) {
        twi_read_byte(_gyro_addr, GYRO_STATUS_REG, &status);
      }

      int16_t _x=gyro_readAxis(GYRO_OUT_X);
      int16_t _y=gyro_readAxis(GYRO_OUT_Y);
